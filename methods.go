...
// SendResult mirrors the "result" object of a successful sendNow response.
...
// username and password. Most callers never need this directly: Send obtains
// and caches a token automatically.
...
// Send delivers one message to the given numbers. Numbers are normalized
// first (spaces and the international "+"/"00" prefix removed) because the
// gateway rejects numbers containing spaces or a plus sign. The cached
// integration token is refreshed once if the gateway reports invalid
// authentication.
func (c *Client) Send(numbers []string, message string) (*SendResult, error) {
	...
	result, badAuth, err := c.sendOnce(token, normalized, message)
	if badAuth {
		if token, err = c.getToken(true); err != nil {
			return nil, err
		}
		result, _, err = c.sendOnce(token, normalized, message)
	}
	return result, err
}

// SendSingle delivers one message to one number.
func (c *Client) SendSingle(number, message string) (*SendResult, error) {
	return c.Send([]string{number}, message)
}

// sendOnce performs one sendNow call. badAuth reports that the gateway
// rejected the integration token, so the caller may refresh and retry.
func (c *Client) sendOnce(token string, numbers []string, message string) (result *SendResult, badAuth bool, err error) {
	...
	req, err := http.NewRequest(http.MethodPost, c.Config.BaseURL+sendPath, bytes.NewReader(payload))
	if err != nil {
		return nil, false, err
	}
